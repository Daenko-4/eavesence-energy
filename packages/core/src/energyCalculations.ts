...
export function calculateEnergyCosts({
  ...
  const estimatedConsumption =
    calculationType === "consumption" ? estimatedKwhPerUse : powerKwhPerUse;
  const kwhPerUse = mode === "estimate" ? estimatedConsumption : measuredKwhPerUse;
  ...
  const isValid = electricityPrice > 0 && usesPerWeek > 0 && hasValidConsumption;
